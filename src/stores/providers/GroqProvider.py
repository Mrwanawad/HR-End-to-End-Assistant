...
class GroqProvider( LLMInterface ):
    ...
    def generate_text( self, system_prompt: str, user_prompt: str, max_output_tokens: int = 2048, temp: float = 0.0) : # type: ignore
        ...
        response = self.client.chat.completions.create( model= self.model_id, messages= messages,                 # type: ignore
                                             max_tokens= max_output_tokens, temperature= temp )   

        return response.choices[0].message.content         
        
    async def analyze_cv( self, messages: List, max_output_tokens: int = 2048, temp: float = 0.0 )  :         # type: ignore
...
if __name__ == '__main__':
    ...
    groq = GroqProvider(
        ...
        model_id= get_settings().GROQ_MODEL
    )
    
    response = groq.generate_text(
        system_prompt= 'Answer Users qs',
        user_prompt= "Can You respond with structured JSON outputs ?"
    )
